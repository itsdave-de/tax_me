...
class UmsatzsteuerVoranmeldung(Document):
	...
	def berechnen(self):
		...
		pi_list = frappe.get_all(
			...
			filters={
				"docstatus": 1,
				"bill_date": ["between", [self.von, self.bis]]
				},
			fields = ["name", "bill_date", "total", "net_total", "total_taxes_and_charges"]
			)
		summe_netto = 0
		summe_steuern = 0
		for pi in pi_list:
			summe_netto_pi += pi["net_total"]
			summe_steuern_pi += pi["total_taxes_and_charges"]
